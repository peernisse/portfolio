...
**Disclaimer: ** *I built all the items shown on this page with exception of the Power BI interface for the SME Search Tool. I did build the SME dataset, but someone else made the cool dashboard in Power BI. I also extend recognition to my mentors within the company over the years.*
...
Can be output to Word (static) or HTML (interactive) with Stantec formatting already applied.
...
The Environmental Services SME Search Tool is a good example. The data are in SharePoint and viewed via an interactive Power BI Report. I set up the SME dataset, but did not do the Power BI dashboard on this one.

[ES SME Tool](https://stantec.sharepoint.com/sites/EnvironmentalServices/SitePages/Subject-Matter-Experts-Search-Tool.aspx)



# Internal Web Applications with Shiny
Try my, still in progress, R Shiny App for exploring datasets. Follow the link, hit the "Load Demo Data" button, pick a few locations and analytes from the left side, and explore the demo data.

[My Demo Water Quality Exporer App](https://peernisse.shinyapps.io/wqdashboard/)
<hr/>
